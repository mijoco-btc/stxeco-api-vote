...
docker tag mijoco/stxeco_api_vote stxeco_api_vote
docker push mijoco/stxeco_api_vote:latest

#docker pull mijoco/stxeco_api_vote;
docker rm -f stxeco_api_vote  
docker run -d -t -i --network host --env-file ~/.env --name $DOCKER_NAME -p 6060:6060 mijoco/stxeco_api_vote:latest
