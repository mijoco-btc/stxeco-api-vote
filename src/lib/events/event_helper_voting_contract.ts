...
import { VotingEventVoteOnProposal, VotingEventConcludeProposal, VotingEventProposeProposal, ExtensionType, getTransaction, ProposalContract, StackerProposalData } from "@mijoco/stx_helpers/dist/index";
...
export async function readVotingEvents(genesis: boolean, daoContract: string, votingContract: string) {
  console.log("VotingContractEvents: genesis " + genesis);
  console.log("VotingContractEvents: daoContract " + daoContract);
...
async function processEvent(event: any, daoContract: string, votingContract: string) {
  ...
  if (result.value.event.value === "propose") {
    ...
    //console.log('processEvent: extension: ', util.inspect(event, false, null, true /* enable colors */))
    const votingContractEvent = {
      _id: new ObjectId(),
      event: "vote",
      event_index: Number(event.event_index),
    ...
    //console.log('processEvent: extension: enabled=' + votingContractEvent.enabled + ' contract=' + votingContractEvent.extension + ' contract=' + votingContractEvent.extension + ' event.event_index=' + event.event_index)
    await saveOrUpdateEvent(votingContractEvent);
    ...
    //console.log("processEvent: extension: enabled=" + votingContractEvent + " contract=" + votingContractEvent.extension + " contract=" + votingContractEvent.extension + " event.event_index=" + event.event_index);
    await saveOrUpdateEvent(votingContractEvent);
  }
}
...
export async function fetchAllConcludedEvents(): Promise<any> {
  ...
  console.log("fetchAllConcludedEvents: ", result);
  return result;
}
