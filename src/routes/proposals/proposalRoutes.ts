...
import {
  fetchActiveProposeEvents,
  fetchAllConcludedEvents,
  fetchAllProposeEvents,
  fetchLatestProposal,
  fetchProposeEvent,
  toggleSipStatus,
  updateStackerData,
} from "../../lib/events/event_helper_voting_contract";
import {
  generateAddresses,
  generateAddressesV2,
  generateBitcoinAddressV2,
  generateStacksAddressV2,
  getFunding,
  getMetaData,
} from "../../lib/events/proposal";
import {
  fetchTentativeProposals,
  findTentativeProposalByContractId,
  saveOrUpdateTentativeProposal,
} from "../../lib/data/db_models";
import { getConfig } from "../../lib/config";

const router = express.Router();

router.get("/generate-bitcoin-addresses/:msg", async (req, res, next) => {
  try {
    const extensions = await generateBitcoinAddressV2(req.params.msg);
...
router.get("/generate-stacks-addresses/:msg", async (req, res, next) => {
  try {
    const extensions = await generateStacksAddressV2(req.params.msg);
    return res.send(extensions);
  } catch (error) {
    console.log("Error in routes: ", error);
    next("An error occurred fetching pox-info.");
...
router.get(
  "/generate-stacker-data/:sip/:proposalId",
  async (req, res, next) => {
    try {
      const proposal = await updateStackerData(
        Boolean(req.params.sip),
...
router.get("/get-executed-proposals/:daoContract", async (req, res, next) => {
  try {
    const extensions = await fetchByBaseDaoEvent(
      req.params.daoContract,
      "execute"
    );
    return res.send(extensions);
  } catch (error) {
...
router.get("/get-proposal/:proposalContractId", async (req, res, next) => {
  try {
    const proposal: VotingEventProposeProposal = await fetchLatestProposal(
      req.params.proposalContractId
    );
...
router.get(
  "/get-funding/:submissionContract/:proposalContract",
  async (req, res, next) => {
    try {
      const funding: FundingData = await getFunding(
        req.params.submissionContract,
        req.params.proposalContract
      );
...
router.get("/get-tentative-proposal/:proposalId", async (req, res, next) => {
  try {
    const proposal = await findTentativeProposalByContractId(
      req.params.proposalId
    );
    return res.send(proposal);
  } catch (error) {
...
router.get(
  "/create-tentative-proposal/:start/:end/:proposalId",
  async (req, res, next) => {
    try {
      const deployed = await lookupContract(
        getConfig().stacksApi,
        req.params.proposalId
      );
