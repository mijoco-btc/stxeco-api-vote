...
import { getAddressFromHashBytes, getHashBytesFromAddress } from "./pox_helper";
import { Delegation, Stacker, StackerInfo, callContractReadOnly } from "@mijoco/stx_helpers/dist/index";
...
async function getCheckDelegation(address:string):Promise<Delegation|undefined> {
  try {
    const functionArgs = [`0x${hex.encode(serializeCV(principalCV(address)))}`];
    const data = {
      contractAddress: getConfig().poxContractId!.split('.')[0],
    ...
    const val = await callContractReadOnly(getConfig().stacksApi, data);
