...
async function getNftHoldingsByAssetAndPage(
  stxAddress: string,
...
export async function getNftHoldings(
  ...
  do {
    events = await getNftHoldingsByAssetAndPage(
      stxAddress,
      assetId,
      limit,
      offset
    ...
    if (events && events.total > 0) {
      for (const event of events.results) {
        ...
        if (result.type === "uint") {
          ...
          token = {
            id: Number(result.value["token-id"].value),
...
const trait =
  '{"maps":[],"functions":[{"args":[{"name":"sender","type":"principal"}],"name":"execute","access":"public","outputs":{"type":{"response":{"ok":"bool","error":"uint128"}}}}],"variables":[],"fungible_tokens":[],"non_fungible_tokens":[]}';
export async function getProposalsByTrait() {
  const url =
    getConfig().stacksApi + "/extended/v1/contract/by_trait?trait_abi=" + trait;
  ...
  try {
    do {
      ...
      const ourProps = val.results.filter(
        (o: any) => o.contract_id.indexOf(".edp") > -1
      );
      if (ourProps && ourProps.length > 0)
        edaoProposals = edaoProposals.concat(ourProps);
      count++;
  ...
  return {
    executiveSignalsRequired: Number(
      (await fetchDataVar(
        ...
        getDaoConfig().VITE_DOA_EMERGENCY_EXECUTE_EXTENSION,
...
export async function getBurnBlockHeightFromHash(
  hash: string
): Promise<number> {
...
export async function isExecutiveTeamMember(
  ...
  if (!stxAddress || stxAddress === "undefined")
    return { executiveTeamMember: false };
  const functionArgs = [
    `0x${hex.encode(serializeCV(principalCV(stxAddress)))}`,
  ];
  const data = {
    contractAddress: getDaoConfig().VITE_DOA_DEPLOYER,
    contractName: getDaoConfig().VITE_DOA_EMERGENCY_EXECUTE_EXTENSION,
...
export async function getEdgBalance(
  stxAddress: string
...
export async function getEdgLocked(
  stxAddress: string
