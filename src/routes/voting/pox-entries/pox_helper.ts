...
export async function getPoxBitcoinAddressInfo(
  ...
  return {
    partialStackedByCycle: await getPartialStackedByCycle(
      getConfig().stacksApi,
      getConfig().network,
...
export async function readPoxEntriesFromContract(cycle: number): Promise<any> {
  const len = await getNumbEntriesRewardCyclePoxList(
    getConfig().stacksApi,
    getConfig().poxContractId!,
    cycle
  );
...
export async function readSavePoxEntries(
  cycle: number,
  len: number,
  offset: number
): Promise<any> {
  ...
  for (let i = offset; i < len; i++) {
    ...
    try {
      const entry = await getRewardSetPoxAddress(
        getConfig().stacksApi,
        getConfig().poxContractId!,
        cycle,
        i
      ...
      console.error(
        "readSavePoxEntries: saving: " + poxAddr + "/" + cycle + "/" + i
      );
...
export async function collatePoolStackerInfo(
  address: string,
  cycle: number
): Promise<StackerStats> {
  ...
  const stackerEvents: Array<PoolStackerEvent> =
    await findPoolStackerEventsByStacker("pox-4", address);
  const stackerEventsAsDelegator: Array<PoolStackerEvent> =
    await findPoolStackerEventsByStacker("pox-4", address);
  ...
  stackerInfoPerCycle.cycleInfo = await getPoxCycleInfo(
    getConfig().stacksApi,
    getConfig().poxContractId!,
    cycle
  );
...
export async function extractAllPoxEntriesInCycle(
  address: string,
  cycle: number
) {
...
export async function collateSoloStackerInfo(
  address: string,
  cycle: number
): Promise<StackerStats> {
  ...
  console.log(
    "collateSoloStackerInfo: address: " + address + " proxy: " + voterProxy
  );
  ...
  let poxEntries: Array<any> = await extractAllPoxEntriesInCycle(
    voterProxy,
    cycle
  );
  const poxEntries1: Array<any> = await extractAllPoxEntriesInCycle(
    voterProxy,
    cycle + 1
  );
  ...
  if (vote && vote.poxStacker) {
    stackerEvents = await findPoolStackerEventsByStacker(
      "pox-4",
      vote.poxStacker
    );
    stackerEventsAsDelegator = await findPoolStackerEventsByStacker(
      "pox-4",
      vote.poxStacker
    );
  }
  ...
  if (hashBytes)
    stackerEventsAsPoxAddress =
      await findPoolStackerEventsByHashBytesAndVersion(
        hashBytes.version,
        hashBytes.hashBytes,
        0,
        100
...
export async function collateStackerInfo(
  address: string,
  cycle: number
): Promise<StackerStats> {
...
export async function findLastPoxEntryByCycle(cycle: number): Promise<any> {
  const result = await poxAddressInfo
    .find({ cycle })
    .sort({ index: -1 })
    .limit(1)
    .toArray();
...
export async function findPoxEntryByCycleAndIndex(
  cycle: number,
  index: number
): Promise<any> {
...
export async function findPoxEntriesByAddressAndCycle(
  address: string,
  cycle: number
): Promise<any> {
  const result = await poxAddressInfo
    .find({ bitcoinAddr: address, cycle })
    .toArray();
  return result;
}
...
export async function findPoxEntriesByStackerAndCycle(
  stacker: string,
  cycle: number
): Promise<any> {
...
export async function saveOrUpdatePoxEntry(poxEntry: PoxEntry) {
  try {
    console.log(
      "saveOrUpdatePoxEntry: saving: " +
        poxEntry.bitcoinAddr +
        "/" +
        poxEntry.stacker +
        "/" +
        poxEntry.cycle +
    ...
    console.log(
      "saveOrUpdatePoxEntry: unable to save or update" + poxEntry.bitcoinAddr
    );
  }
}
